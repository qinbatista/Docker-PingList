...
class PingList:
    def __init__(self):
        self.__target_server_domain_name_list = ["timov4.qinyupeng.com","us.qinyupeng.com"]
        ...
        while True:
            for target_domain in self.__target_server_domain_name_list:
                try:
                    ...
                    self.__log(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}][{target_domain}:{self.__target_server_ipv4}] message: {this_docker_ipv4}, can connect:{str(self._can_connect)}")
